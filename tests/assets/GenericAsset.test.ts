...
describe('GenericAsset', () => {
    ...
    beforeEach(() => {
        ...
        Object.defineProperty(global, 'window', {
            value: {
                __EPICURRENTS__: { APP: mockApp, EVENT_BUS: mockEventBus, RUNTIME: null }
            } as any,
            writable: true,
        })
    ...
    describe('property setters and getters', () => {
        let asset: InstanceType<typeof TestAsset>
        ...
        describe('isActive', () => {
            it('should set active state and dispatch events', () => {
                ...
                expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                    AssetEvents.ACTIVATE, asset.id, 'before',
                    expect.objectContaining({ origin: asset })
                )
                expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                    AssetEvents.ACTIVATE, asset.id, 'after',
                    expect.objectContaining({ origin: asset })
                )
            })
            ...
            it('should dispatch deactivate events when set to false', () => {
                ...
                expect(asset.isActive).toBe(false)
                expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                    AssetEvents.DEACTIVATE, asset.id, 'before',
                    expect.objectContaining({ origin: asset })
                )
                expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                    AssetEvents.DEACTIVATE, asset.id, 'after',
                    expect.objectContaining({ origin: asset })
                )
            })
        })
    ...
    describe('event handling methods', () => {
        ...
        describe('dispatchEvent', () => {
            ...
            it('should support custom phase and detail', () => {
                ...
                expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                    'test-event', asset.id, 'before',
                    expect.objectContaining({ origin: asset, customData: 'value' })
                )
            })
        })
        ...
        describe('dispatchPayloadEvent', () => {
            it('should dispatch event with payload', () => {
                ...
                expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                    'test-event', asset.id, 'after',
                    expect.objectContaining({ origin: asset, payload })
                )
            })
        })
        ...
        describe('dispatchPropertyChangeEvent', () => {
            it('should dispatch property change event', () => {
                ...
                expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                    'property-change:name', asset.id, 'after',
            ...
            it('should use current property value when values not provided', () => {
                ...
                expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                    ...
                    expect.objectContaining({
                        origin: asset, property: 'name',
                        newValue: asset.name, oldValue: asset.name
                    })
                )
            })
        ...
        describe('onPropertyChange', () => {
            it('should add property change listener for single property', () => {
                ...
                expect(mockEventBus.addScopedEventListener).toHaveBeenCalledWith(
                    ...
                    'test-subscriber', asset.id, 'after'
                )
            })
            ...
            it('should add property change listeners for multiple properties', () => {
                ...
                expect(mockEventBus.addScopedEventListener).toHaveBeenCalledWith(
                    'property-change:modality', expect.any(Function),
                    'test-subscriber', asset.id, 'after'
                )
            })
        })
    ...
    describe('serialize', () => {
        it('should return serialized asset data', () => {
            ...
            expect(serialized).toEqual({ id: asset.id, modality: 'test', name: 'Test Asset' })
        })
    })
    ...
    describe('subscription methods', () => {
        ...
        describe('unsubscribeAll', () => {
            it('should call event bus unsubscribeAll method', () => {
                asset.unsubscribeAll('test-subscriber')
    ...
    describe('getEventHooks', () => {
        it('should call event bus getEventHooks method', () => {
            const asset = new TestAsset('Test Asset', 'test')
            asset.getEventHooks('test-event', 'test-subscriber')
            expect(mockEventBus.getEventHooks).toHaveBeenCalledWith(
                'test-event', 'test-subscriber', asset.id
            )
        })
    })

    describe('state', () => {
        it('should default to added', () => {
            const asset = new TestAsset('Test Asset', 'test')
            expect(asset.state).toBe('added')
        })

        it('should reset error reason when transitioning from error', () => {
            const asset = new TestAsset('Test Asset', 'test')
            asset.state = 'error'
            asset.errorReason = 'Something failed'
            asset.state = 'added'
            expect(asset.errorReason).toBe('')
        })

        it('should not reset error reason when staying in error', () => {
            const asset = new TestAsset('Test Asset', 'test')
            asset.state = 'error'
            asset.errorReason = 'Something failed'
            asset.state = 'error'
            expect(asset.errorReason).toBe('Something failed')
        })
    })

    describe('destroy', () => {
        it('should deactivate asset, remove listeners, dispatch DESTROY, and set state to destroyed', () => {
            const asset = new TestAsset('Test Asset', 'test')
            asset.isActive = true
            mockEventBus.dispatchScopedEvent.mockClear()
            asset.destroy()
            expect(asset.isActive).toBe(false)
            expect(asset.state).toBe('destroyed')
            expect(mockEventBus.removeScope).toHaveBeenCalledWith(asset.id)
            expect(mockEventBus.dispatchScopedEvent).toHaveBeenCalledWith(
                AssetEvents.DESTROY, asset.id, 'after',
                expect.objectContaining({ origin: asset })
            )
        })
