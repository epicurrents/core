...
export default abstract class GenericBiosignalResourceSAB extends GenericResource implements BiosignalResource {
    ...
    constructor (name: string, sensitivity: number, type: string, memoryManager: MemoryManager, source?: StudyContext) {
    ...
    async getAllRawSignals (range: number[], config?: any): Promise<SignaCacheResponse | null> {
